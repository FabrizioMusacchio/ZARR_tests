...
# ...to groups:
zarr_out_root["sub_arrays"].attrs["description"] = "A test sub-group"

# ...and to arrays:
zarr_out_root["sub_arrays/array_1D_1"].attrs["description"] = "A test array"

for key in zarr_out_root.attrs.keys():
    print(f"{key}: {zarr_out_root.attrs[key]}")
print(zarr_out_root["sub_arrays"].attrs["description"])
print(zarr_out_root["sub_arrays/array_1D_1"].attrs["description"])
# %% DISTRIBUTED STORAGE
...
# %% ZARR ADVANCED
zarrstore = zarr.DirectoryStore("zarr_array.zarr")
zarr_ds   = zarr.group(store=zarrstore, overwrite=True)
zarr_out_3D = zarr_ds.create_dataset("array_3D", data=array_3D)
zarr_out_2D = zarr_ds.create_dataset("array_2D", data=array_2D)

zarr_out_3D.attrs["shape"]  = array_3D.shape
zarr_out_3D.attrs["author"] = "Pixel Tracker"
zarr_out_3D.attrs["date"]   = "Oct 13, 2022"

for key in zarr_out_3D.attrs.keys():
    print(f"{key}: {zarr_out_3D.attrs[key]}")

compressor=Blosc(cname='zstd', clevel=0, shuffle=Blosc.SHUFFLE)
zarr_out = zarr_ds.create_dataset("array_3D_compress_level_0", data=array_3D, compressor=compressor,overwrite=True)

compressor=Blosc(cname='zstd', clevel=1, shuffle=Blosc.SHUFFLE)
zarr_out = zarr_ds.create_dataset("array_3D_compress_level_1", data=array_3D, compressor=compressor,overwrite=True)

compressor=Blosc(cname='zstd', clevel=5, shuffle=Blosc.SHUFFLE)
zarr_out = zarr_ds.create_dataset("array_3D_compress_level_5", data=array_3D, compressor=compressor,overwrite=True)

dump = zarr.open("zarr_array.zarr")
print(type(dump["array_3D"]))
print(type(dump["array_3D"][:]))

dask_zarr_array = da.from_zarr("zarr_array.zarr", component="array_3D")

zarr_out = z2.create_dataset("array_3D", data=array_3D, chunks=chunks_use, compressor=compressor)
